...
- **sides.*.units[].position** — `[x, y]` on the battlefield grid (currently 12 wide × 8 tall, 0-indexed from top-left). Positions are RULE-BEARING (ADR 0007): 1 square = 2", distance is the Chebyshev (king's-move) count × 2", and inches floor into squares. A shooting target must be within the weapon's range — the loader rejects an out-of-range scripted shot, and the player's target menu only offers reachable, unengaged targets. Adjacent squares (diagonals count) = the 2" engagement range; engaged units can neither shoot nor be shot at (10.04, 04.02), so keep shooters and their targets clear of any standing combat. The 24"+ guns cover the whole 12×8 board; only the 18" guns (Shoota, Fleshborer) can actually be out-ranged on it — mind them.
...
- **turns** — explicit list of turns. Phases: `"shooting"` (one side acts) and `"fight"` (BOTH sides act: every engaged unit must fight, players alternate picks, and `active_side` names whose turn it is — that side picks first). A fight turn requires at least one engaged starting pair: opposing units on adjacent squares (diagonals count — `core.scenario.in_engagement_range`). In a fight turn the `actions` array scripts the *opponent's* picks only (the player picks their own fights; the loader rejects player-side fight actions), each action naming a **melee** weapon the unit carries. Loadout overrides never disarm a unit in melee — an override that names only guns falls back to the sheet's melee weapons. A heuristic opponent fights for itself — under `opponent_strategy: "heuristic"`, leave fight turns' `actions` empty (the loader rejects them, since fight scripts always belong to the opponent).
...
3. **Write the JSON.** Use an existing scenario as a template. Position units so the relevant weapons are in range — this is now checked, not trusted: the loader fails a scripted out-of-range shot with the distance and the range in the message, and an unshootable setup surfaces as an empty/short menu when you play it.
